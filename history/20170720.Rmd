...
# 表示使用連續型時間計算的數值格式 (ct: continuous time)
t1 = as.POSIXct(appledaily$dt,format = '%Y年%m月%d日%H:%M')
class(t1)
unclass(t1)

# 表示使用可讀的列表物件 (lt: legibletime)
t2 = as.POSIXlt(appledaily$dt,format = '%Y年%m月%d日%H:%M')
class(t2)
unclass(t2)

# Date 和 POSIX 差別
# Date 類別表示 "日期",  表示距離1970/1/1多少天, 單位為天
# POSIX 類別表示 "時間", 表示距離1970/1/1多少秒, 單位為秒
...
applenews %>%
  group_by(category) %>% 
  summarise_at(.vars=vars(clicked),.funs=funs(sum,mean))
>>>>>>> 818279985187caa7316f2cb18e17e100df2164aa:history/20170720.Rmd
